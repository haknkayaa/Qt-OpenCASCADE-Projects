...
void MainWindow::createMiddleWidget() {
    ...
    {   // Information Dock Widget
        ...
        information_partName = new QLineEdit("Example");
        ...
        information_parentName = new QLineEdit("Example");
        information_parentName->setReadOnly(true);
        informationLayout->addWidget(information_parentName, 1, 1);

        informationLayout->addWidget(new QLabel("Transparency"), 2, 0);
        information_transparencySlider = new QSlider(Qt::Horizontal);
        informationLayout->addWidget(information_transparencySlider, 2, 1);

        informationLayout->addWidget(new QLabel("Color"), 3, 0);
        information_colorButton = new QPushButton("..");
        informationLayout->addWidget(information_colorButton, 3, 1);

        informationLayout->addWidget(new QLabel("Material"), 4, 0);
        information_materialSelect = new QComboBox();
        information_materialSelect->addItems(QStringList() << "ALUMINIUM" << "GOLD");
        informationLayout->addWidget(information_materialSelect, 4, 1);
...
void MainWindow::modelTreeItemClicked(QTreeWidgetItem *arg_item) {
   ...
   findItemFromUploadedObjects(arg_item, mySTEPProcessor->modelTree);
}

void MainWindow::findItemFromUploadedObjects(QTreeWidgetItem *arg_item, vector<AssemblyNode> arg_modelTree) {
    ...
    for (int i = 0; i < arg_modelTree.size(); ++i) {
        if (arg_item == arg_modelTree[i].treeWidgetItem) {
            ...
            information_partName->setText(arg_modelTree[i].Name);
            //information_parentName->setText(arg_modelTree[i].Parent->Name);
            //information_transparencySlider->setValue(arg_modelTree[i].transparency);
            information_colorButton->setText("arg_modelTree[i].color");
        }

        findItemFromUploadedObjects(arg_item, arg_modelTree[i].Children);
    }
}
