...
//!TODO!: Şekil fonksiyonları model tree ye entegre edilecek
void MainWindow::cube() {
    bool ok;
    QList<double> list = InputDialog::getFloats(this, &ok, 3);
    if (list.size() == 3) {
        auto newShape = make_shared<AssemblyNode>();
        //newShape->Label = "";
        ...
        //AssemblyNode newShape;
        //shared_ptr<AssemblyNode> newShape = make_shared<AssemblyNode>();

        newShape->Name = QString("Cube %1").arg(numberOfShapes[0]);
        newShape->topoShape = aTopoBox;
        newShape->shape = anAisBox;

        auto *item = new QTreeWidgetItem();
        item->setText(0, newShape->Name);
        modelTreeWidget->insertTopLevelItem(0, item);
        numberOfShapes[0]++;

        newShape->treeWidgetItem = item;
        createdShapeList.push_back(*newShape);
...
void MainWindow::cylinder() {
    ...
    if (list.size() == 2) {
        ...
        AssemblyNode newShape;
        newShape.Name = QString("Cylinder %1").arg(numberOfShapes[1]);
        newShape.topoShape = aTopoCylinder;
        ...
        newShape.treeWidgetItem = item;
        mySTEPProcessor->modelTree.push_back(newShape);
...
void MainWindow::sphere(){
    ...
    if (list.size() == 1) {
        ...
        AssemblyNode newShape;
        newShape.Name = QString("Sphere %1").arg(numberOfShapes[2]);
        newShape.topoShape = aTopoSphere;
...
void MainWindow::slot_moveTo(){
    ...
    int index = 0;
    for (int i = 0; i < mySTEPProcessor->modelTree.size(); ++i) {
        if(!QString::compare(shapeName, mySTEPProcessor->modelTree[i].Name, Qt::CaseInsensitive))
            index = i;
    }

    if(moveDialog->exec() == QDialog::Accepted)
    {
        //myViewerWidget->moveTo(mySTEPProcessor->modelTree[index].shape, xSlider->value(), ySlider->value(), zSlider->value());
        gp_Trsf transformation;
        transformation.SetTranslation(gp_Pnt(gp_XYZ(0, 0, 0)),
                                      gp_Pnt(gp_XYZ(xSlider->value(), ySlider->value(), zSlider->value())));
        TopoDS_Shape aShape = currentSelectedShape.topoShape;
        ...
        myViewerWidget->getContext()->Redisplay(currentSelectedShape.shape, true);
        myViewerWidget->getContext()->UpdateCurrentViewer();
...
void MainWindow::merge() {
    ...
    if(mergeDialog->exec() == QDialog::Accepted) {
        ...
        if (QString::compare(firstShape->currentText(), secondShape->currentText(), Qt::CaseInsensitive)) {
            ...
            int index1, index2;
            //birlestirilcek olan shapelerin modelTree deki indexini bulur.
            for (int i = 0; i < mySTEPProcessor->modelTree.size(); ++i) {
                int x = QString::compare(mySTEPProcessor->modelTree[i].Name, firstShape->currentText(), Qt::CaseInsensitive);
                int y = QString::compare(mySTEPProcessor->modelTree[i].Name, secondShape->currentText(), Qt::CaseInsensitive);
                if (!x)
                    index1 = i;
                else if (!y)
                    index2 = i;
            ...
            //TopoDS_Shape firstShape = mySTEPProcessor->modelTree[index1].topoShape
            //TopoDS_Shape secondShape = mySTEPProcessor->modelTree[index2].topoShape

            aBuilder.Add(aCompound, mySTEPProcessor->modelTree[index1].topoShape);
            aBuilder.Add(aCompound, mySTEPProcessor->modelTree[index2].topoShape);
            AIS_Shape *anAISShape = new AIS_Shape(aCompound);

            //Birlestirilen shapeleri sahneden ve modelTreeden siler
            myViewerWidget->getContext()->Remove(mySTEPProcessor->modelTree[index1].shape);
            myViewerWidget->getContext()->Remove(mySTEPProcessor->modelTree[index2].shape);
            mySTEPProcessor->modelTree.erase(mySTEPProcessor->modelTree.begin()+index1);
            mySTEPProcessor->modelTree.erase(mySTEPProcessor->modelTree.begin()+index2);
...
void MainWindow::chooseFullBody(){
    ...
    //qDebug() << mySTEPProcessor->modelTree.size();
    for (int i = 0; i < mySTEPProcessor->modelTree.size(); ++i) {
        myViewerWidget->getContext()->Deactivate(mySTEPProcessor->modelTree[i].shape);
        myViewerWidget->getContext()->Activate(mySTEPProcessor->modelTree[i].shape, 0);
    }
}
void MainWindow::chooseVertex(){
    ...
    for (int i = 0; i < mySTEPProcessor->modelTree.size(); ++i) {
        myViewerWidget->getContext()->Deactivate(mySTEPProcessor->modelTree[i].shape);
        myViewerWidget->getContext()->Activate(mySTEPProcessor->modelTree[i].shape, 1);
    }
}
void MainWindow::chooseEdge(){
    ...
    for (int i = 0; i < mySTEPProcessor->modelTree.size(); ++i) {
        myViewerWidget->getContext()->Deactivate(mySTEPProcessor->modelTree[i].shape);
        myViewerWidget->getContext()->Activate(mySTEPProcessor->modelTree[i].shape, 2);
    }
}
void MainWindow::chooseFace() {
    ...
    for (int i = 0; i < mySTEPProcessor->modelTree.size(); ++i) {
        myViewerWidget->getContext()->Deactivate(mySTEPProcessor->modelTree[i].shape);
        myViewerWidget->getContext()->Activate(mySTEPProcessor->modelTree[i].shape, 4);
    }
}
