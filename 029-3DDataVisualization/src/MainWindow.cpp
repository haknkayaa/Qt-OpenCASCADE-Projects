...
void MainWindow::createMenuBar() {
    ...
    connect(lineChartAction, &QAction::triggered, this, &MainWindow::slot_createView);
...
void MainWindow::slot_createView() {

    qDebug() << "slot_createView";
    ...
    dataArray->resize(dataNumber);
    QScatterDataItem *ptrToDataArray = &dataArray->first();


    QVector3D randomPoints;

    for (int i = 0; i < dataNumber ; i++){

        randomPoints.setX((float)(QRandomGenerator::global()->bounded(100)/10.0));
        randomPoints.setY((float)(QRandomGenerator::global()->bounded(100)/10.0));
        randomPoints.setZ((float)(QRandomGenerator::global()->bounded(100)/10.0));

        ptrToDataArray->setPosition(randomPoints);
        ptrToDataArray++;
    }
    ...
    myDialog->setFixedSize(this->size()/2);
    myDialog->show();
