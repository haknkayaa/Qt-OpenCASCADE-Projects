...
bool
DBManager::insertMusic(const QString tableName, const QString &name, const QString &path, const int &lenghtAsSecond,
    ...
    if (!name.isEmpty()) {
        ...
        queryAdd.prepare("INSERT INTO " + tableName +
                         " (name,path,lenghtAsSecond,lenghtToShow) VALUES (:name,:path,:lenghtAsSecond,:lenghtToShow)");
...
bool DBManager::updateMusic(const int &musicId = -1, const QString &name = "", const QString &path = "",
                            const int &lenghtAsSecond = 0, QString lenghtToShow = "00:00") {
    bool success = false;
...
void DBManager::printAllPersons(QString table, QString index) const {
    qDebug() << "Elements in database:";
    QSqlQuery query("SELECT * FROM elements");
    int idName = query.record().indexOf("element");
    while (query.next()) {
...
bool DBManager::personExists(const QString &name) const {
    ...
    checkQuery.prepare("SELECT element FROM elements WHERE element = (:name)");
