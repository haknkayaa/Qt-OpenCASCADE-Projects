...
void MainWindow::slot_informationColorDialog() {
    ...
    if (selectedColor.isValid()) {
        ...
        //updateItemFromUploadedObjects(currentSelectedShape, mySTEPProcessor->modelTree);
    }
}
...
/** current item modeltree içinde arayip güncellemesini ister
 *
...
void MainWindow::slot_showOnlySelectedPart() {
    ...
    myViewerWidget->getContext()->EraseAll(0);
    myViewerWidget->getContext()->UpdateCurrentViewer();
    myViewerWidget->fitAll();

    myViewerWidget->getContext()->Display(currentSelectedShape.shape, 0);
    myViewerWidget->getContext()->UpdateCurrentViewer();
    myViewerWidget->fitAll();
}
