...
// qDebug fonksiyonlarını textbrowser'a yönlendirme
        ...
        switch (type) {
            ...
            case QtCriticalMsg:
                if (MainWindow::text != 0){
                    ...
                    //"Debug: %s (%s:%u, %s)\n", localMsg.constData(), context.file, context.line,
                    //        context.function;
                    QString output = "Debug : " + msg + " : " + context.file + " ; " + context.line;
...
// MainWindow için bazı ilk ayarlar
...
void MainWindow::createMiddleWidget() {
    ...
    {   // MODEL TREE DOCK WİDGET
        ...
        QTreeWidget *modelTree = new QTreeWidget(dockWidget_modelTree);

        QStringList headerLabels = {"Part Name"};
        modelTree->setHeaderLabels(headerLabels);

        dockWidget_modelTree->setWidget(modelTree);
        addDockWidget(Qt::LeftDockWidgetArea, dockWidget_modelTree);
        tabifiedDockWidgets(dockWidget_modelTree);
    }   // MODEL TREE DOCK WİDGET

    {   // Information Dock Widget
        QDockWidget *dockWidget_modelTree2 = new QDockWidget("Information", this);

        QTreeWidget *modelTree = new QTreeWidget(dockWidget_modelTree2);

        QStringList headerLabels = {"Label", "Value"};
        modelTree->setHeaderLabels(headerLabels);


        dockWidget_modelTree2->setWidget(modelTree);
        addDockWidget(Qt::LeftDockWidgetArea, dockWidget_modelTree2);
        tabifiedDockWidgets(dockWidget_modelTree2);
    }   // Information Dock Widget
    ...
    { // CONSOLE
        ...
        QTextBrowser *text = new QTextBrowser(dockWidget_terminal);
        dockWidget_terminal->setWidget(text);
